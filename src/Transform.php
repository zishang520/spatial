...
class Transform
{
    ...
    protected const E2_WGS84 = -(self::F_WGS84 * self::F_WGS84);
    ...
    protected const E2_CGCS2000 = -(self::F_CGCS2000 * self::F_CGCS2000);
     ...
     *
     * @param PointWGS84 $pointWGS84 WGS84 坐标点
     * @param array $helmert 七参数（Helmert）变换参数，默认零变换
     */
    public static function WGS84_CGCS2000(PointWGS84 $pointWGS84, array $helmert = self::DEFAULT_HELMERT_PARAMS_WGS84_TO_CGCS2000): PointCGCS2000
    {
        return new PointCGCS2000(self::geoHelmert($pointWGS84, ['a' => self::A_WGS84, 'e2' => self::E2_WGS84], ['a' => self::A_CGCS2000, 'e2' => self::E2_CGCS2000], $helmert));
    }

    /**
     * CGCS2000 坐标系转 WGS84 坐标系。
     *
     * @param PointCGCS2000 $pointCGCS2000 CGCS2000 坐标点
     * @param array $helmert 七参数（Helmert）变换参数，默认零变换
     */
    public static function CGCS2000_WGS84(PointCGCS2000 $pointCGCS2000, array $helmert = self::DEFAULT_HELMERT_PARAMS_CGCS2000_TO_WGS84): PointWGS84
    {
        return new PointWGS84(self::geoHelmert($pointCGCS2000, ['a' => self::A_CGCS2000, 'e2' => self::E2_CGCS2000], ['a' => self::A_WGS84, 'e2' => self::E2_WGS84], $helmert));
    }
     ...
     * @param PointWGS84 $pointWGS84 WGS84 坐标点
     */
    public static function WGS84_GCJ02(PointWGS84 $pointWGS84): PointGCJ02
    {
        if (self::outChina($pointWGS84)) {
            return new PointGCJ02($pointWGS84->getCoordinates());
        }
        [$lng, $lat, $h] = [$pointWGS84->getLongitude(), $pointWGS84->getLatitude(), $pointWGS84->getAltitude()];
     ...
     *
     * @param PointGCJ02 $pointGCJ02 GCJ02 坐标点
     */
    public static function GCJ02_WGS84(PointGCJ02 $pointGCJ02): PointWGS84
    {
        if (self::outChina($pointGCJ02)) {
            return new PointWGS84($pointGCJ02->getCoordinates());
        }
        [$lng, $lat, $h] = [$pointGCJ02->getLongitude(), $pointGCJ02->getLatitude(), $pointGCJ02->getAltitude()];
     ...
     * @param PointGCJ02 $pointGCJ02 GCJ02 坐标点
     */
    public static function GCJ02_BD09(PointGCJ02 $pointGCJ02): PointBD09
    {
        [$lng, $lat, $h] = [$pointGCJ02->getLongitude(), $pointGCJ02->getLatitude(), $pointGCJ02->getAltitude()];
     ...
     * @param PointWGS84 $pointWGS84 WGS84 坐标点
     */
    public static function WGS84_BD09(PointWGS84 $pointWGS84): PointBD09
    {
        return self::GCJ02_BD09(self::WGS84_GCJ02($pointWGS84));
    }

    /**
     * BD09（百度坐标系）转 GCJ02（火星坐标系）。
     *
     * @param PointBD09 $pointBD09 BD09 坐标点
     */
    public static function BD09_GCJ02(PointBD09 $pointBD09): PointGCJ02
    {
        [$lng, $lat, $h] = [$pointBD09->getLongitude(), $pointBD09->getLatitude(), $pointBD09->getAltitude()];
        $x = $lng - 0.0065;
        $y = $lat - 0.006;
     ...
     * @param PointBD09 $pointBD09 BD09 坐标点
     */
    public static function BD09_WGS84(PointBD09 $pointBD09): PointWGS84
    {
        return self::GCJ02_WGS84(self::BD09_GCJ02($pointBD09));
    }
     ...
     */
    public static function CGCS2000_GK(PointCGCS2000 $pointCGCS2000, ?int $w = 3): PointGK
    {
        [$lng, $lat] = [$pointCGCS2000->getLongitude(), $pointCGCS2000->getLatitude()];
        $phi = deg2rad($lat);
        $lam = deg2rad($lng);
     ...
     *
     * @param PointGK $pointGK GK投影坐标点（含带号）
     * @param int $w 分带宽度，3 或 6，默认3度带
     */
    public static function GK_CGCS2000(PointGK $pointGK, int $w = 3): PointCGCS2000
    {
        [$e, $n, $z] = [$pointGK->getLongitude(), $pointGK->getLatitude(), $pointGK->getAltitude()];
        $a = self::A_CGCS2000;
        $e2 = self::E2_CGCS2000;
        $ep2 = self::E_PRIME2_CGCS2000;
        $L0 = deg2rad($w === 6 ? $z * 6 - 3 : $z * 3);
    ...
    {
        if (($from = $point->getCoordinateSystem()) === $coordinateSystemEnum) {
            return $point;
        }
        if (! method_exists(static::class, $method = sprintf('%s_%s', $from->name, $coordinateSystemEnum->name))) {
            throw new \InvalidArgumentException("Conversion type [{$from->name}] to [{$coordinateSystemEnum->name}] is not supported. Supported types: BD09, WGS84, GCJ02.");
        }
    ...
    private static function dLat(float $lng, float $lat): float
    {
        ...
        return $ret + (160 * sin($lat / 12 * M_PI) + 320 * sin($lat * M_PI / 30)) * 2 / 3;
    }
    ...
    private static function dLng(float $lng, float $lat): float
    {
        ...
        return $ret + (150 * sin($lng / 12 * M_PI) + 300 * sin($lng / 30 * M_PI)) * 2 / 3;
    }
     ...
     */
    private static function mArc(float $phi, float $a, float $e2): float
    {
